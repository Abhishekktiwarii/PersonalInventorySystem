...
//    out.println("<p class='name'>"+name+"</p> ");
        ...
        out.println("<legend style=\"font-size: 3vw; font-weight:600\">Income <span id='span'>Category </span></legend>");
        out.println("<form action='AddIncCat'>");
        
        out.println("<center><table style=\"font-size: 2vw;\">");
        out.println("<tr><td>Category name</td><td> : </td><td><input type='text' name='cn' ></td></tr>");
        out.println("<tr><td>Category detail </td><td> : </td><td><input type='text' name='cd'></td></tr>");
        out.println("<tr><td> </td><td></td><td><input type=\"submit\" value=\"Submit\"><input type=\"reset\" value=\"Reset\"></td></tr>");
        out.println("</table></center>");
        
        
        out.println("<hr size='6' color='gray' style='margin:1vw;'>");
        
        IncomeCategouryBean eb = new IncomeCategouryBean();
            IncomeCategoryDAO ed = new IncomeCategoryDAO();
        int id = ub.getUserid();
        ArrayList<IncomeCategouryBean> al = ed.findAll(id);
        
        out.println("<center id='centr'><table width='100%' style='font-size:2vw; border:2;color:white;' class='table'>");
        
        out.println("<tr class='table'><th class='table'>Category Name</th><th class='table'>Category Details</th><th class='table'>Edit</th><th class='table'>Delete</th>");
        for(IncomeCategouryBean a:al){
            out.println("<tr class='table'><td class='table'>"+a.getInc_catname()+"</td><td class='table'>"+a.getInc_catdetails()+"</td><td class='table'><a href='UpdateIncomeCategory?id="+a.getInc_catid()+"'>Edit<a/></td><td class='table'><a href='DeleteIncome?id="+a.getInc_catid()+"'>Delete</a></td></tr>");
        }
        out.println("</table></center>");
        out.println("</form>");

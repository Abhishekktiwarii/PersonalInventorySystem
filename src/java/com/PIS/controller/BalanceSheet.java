...
//    out.println("<p class='name'>"+name+"</p> ");
        ...
        out.println("<legend style=\"font-size: 3vw; font-weight:600\">Bank <span id='span'>Book </span></legend>");
        
            out.println("<center><form action='BalanceSheet'>");
            out.println("<table><tr><td>DateFrom<br><input type='date' name='start'></td><td>To<br><input type='date' name='end'></td><td><br><input type='submit' value='Show'></td>");
            out.println("</form></center>");
            out.println("<div class='outertable'");
            
            
            out.println("<div id='table1'><tr style='color:white;'><th>Incomes</th><th></th></tr><tr style='color:black;'><td>Incomes</td><td>Amount</td></tr>");
            for(IncomeBean h:q){
            out.println("<tr><td style='color:white;'>"+h.getInc_ac()+"</td><td style='color:white;'>"+h.getAmount()+"</td></tr>");
            }
            double sd = p.findAmountById(sdate, edate, x);
            out.println("<tr style='color:white;'><td>Total</td><td>"+sd+"</td></tr>");
            out.println("</div>");
            
            
            
            out.println("<div id='table2'><tr style='color:white;'><th>Expenses</th><th></th></tr><tr style='color:black;'><td>Expenses</td><td>Amount</td></tr>");
            for(ExpensesBean k:al){
            out.println("<tr><td>"+k.getExp_ac()+"</td><td>"+k.getAmount()+"</td></tr>");
            }
            double ab =  ed.findAmountById(sdate, edate, x);
            out.println("<tr style='color:white;'><td>Total</td><td>"+ab+"</td></tr>");
            out.println("</table></div>");
            out.println("</div>");
            out.println("<center><table style='margin-top:10%;'><tr style='color:white;'><th>Gross Profit : </th><th>"+Math.round(sd-ab)+"</th></tr></table></center>");
